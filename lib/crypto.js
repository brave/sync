 ...
 * device_id || count_length || count || padding
 * where device_id and count_length is 1 byte each, and count is
 * count_length bytes.
 * @param {number} deviceId number between 0 and 255, unique for each device
 * @param {number} count the number of records encrypted by this device
 * @returns {Uint8Array}
 */
module.exports.getNonce = function (deviceId/* : number */, count/* : number */) {
  if (typeof count !== 'number' || count < 0) {
    throw new Error('Invalid nonce count')
  }
  if (typeof deviceId !== 'number' || deviceId < 0 || deviceId > 255) {
    throw new Error('Invalid device ID')
  }
  var countLength = Math.ceil(count.toString(16).length / 2)
  if (countLength > nacl.secretbox.nonceLength - 2) {
    throw new Error('Nonce count is too high')
  }
  ...
  var i = 1
  while (i < countLength) {
    nonce[1 + i] = Math.floor(count / Math.pow(256, countLength - i))
    i++
  }
  nonce[1 + countLength] = count % 256
  return nonce
}
...
module.exports.encrypt = function (message/* : Uint8Array */,
  secretboxKey/* : Uint8Array */, deviceId/* : number */, count/* : number */) {
  const nonce = module.exports.getNonce(deviceId, count)
  return {
    nonce,
