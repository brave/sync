 ...
 * @param {string} platform // Requesting platform : android | laptop | ios
 * @returns {Promise}
 */
module.exports.listNotifications = function (SQS, options, category, prefix, platform) {
  return new Promise((resolve, reject) => {
    ...
    SQS.getQueueAttributes(queueAttributesParams, (errorAttr, dataAttr) => {
      if (errorAttr) {
        ...
        listNotificationsRecursively(SQS, options, category, prefix, platform, currentCount, maxCount,
...
function listNotificationsRecursively (SQS, options, category, prefix, platform, currentCount, maxCount, currentContent, currentEntriesToDelete, callback) {
  SQS.receiveMessage(options, (error, data) => {
    if (error) {
      ...
      for (let message of data.Messages) {
        ...
        if (deleteMessage || platform === 'android' || platform === 'ios') { // We delete all notifications for Android and iOS currently.
          let entryToDelete = {
      ...
      if (currentCount >= maxCount) {
        ...
        listNotificationsRecursively(SQS, options, category, prefix, platform, currentCount,
