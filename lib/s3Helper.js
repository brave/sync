  ...
  return new Promise((resolve, reject) => {
    ...
    SQS.getQueueAttributes(queueAttributesParams, (errorAttr, dataAttr) => {
      if (errorAttr) {
        ...
        if (0 == dataAttr.Attributes.ApproximateNumberOfMessages) {
          resolve({
            contents: content
          })
...
function listNotificationsRecursively(SQS, options, category, prefix, platform, currentCount,
  ...
  SQS.receiveMessage(options, (error, data) => {
    if (error) {
      ...
      for (let message of data.Messages) {
        ...
        if (deleteMessage
            || platform === 'android'
            || platform === 'ios') {// We delete all notifications for Android and iOS currently.
          let entryToDelete = {
